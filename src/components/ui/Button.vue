...
const props = defineProps({
  ...
  variant: {
    type: String,
    default: 'primary',
    validator: (value: string) => ['primary', 'secondary', 'tetiary','danger'].includes(value)
  },
  width: {
    type: String,
    default: 'w-auto',
...
const buttonClass = computed(() => {
  switch (props.variant) {
    ...
    default:
      return 'bg-transparent text-blue-600 border-2 border-blue-600 hover:bg-blue-600 hover:text-white focus-visible:outline-blue-600'
  }
})
</script>
...
<template>
  <button
    :class="[
      'flex justify-center rounded-md px-4 py-1.5 text-sm font-medium leading-6 shadow-sm focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2',
      buttonClass,
      width,
